...
float4 PixelShaderFunction(VertexShaderOutput input) : COLOR0
{
	...
	specularSum = specularSum +  (float4(Light0SpecularColor, 1) * specular) * SpecularToggle;
		
	diffuse = saturate(dot(-Light1Direction, normal));
	reflect = normalize(2 * diffuse * normal - float4(Light1Direction, 1));
	specular = pow(saturate(dot(reflect, input.View)), 32);
	diffuseSum = diffuseSum + float4(Light1DiffuseColor, 1) * diffuse * DiffuseIntensity;
	specularSum = specularSum +  float4(Light1SpecularColor, 1) * specular * SpecularToggle;
	...
	specularSum = specularSum +  float4(Light2SpecularColor, 1) * specular * SpecularToggle;
